...
class WildController extends AbstractController
{
    ...
    public function index(ProgramRepository $programRepository, Request $request)
    {
        ...
        $hyphenizedCategories = [];
        $hyphenizedPrograms = [];
        foreach($programs as $program) {
            $category = $program->getCategory();
            $hyphenizedCategories[] = strtolower(str_replace(' ', '-', $category->getName()));
            $hyphenizedPrograms[] = strtolower(str_replace(' ', '-', $program->getTitle()));
        }
        ...
        return $this->render('wild/index.html.twig',
          [
              'programs' => $programs,
              'hyphenizedCategories' => $hyphenizedCategories,
              'hyphenizedPrograms' => $hyphenizedPrograms
          ]
        );
    }
    ...
    public function show(?string $slug, ProgramRepository $programRepository)
    {
        ...
        return $this->render('wild/show.html.twig', [
            'showName' => $showName,
            'program' => $program,
            'hyphenizedCategory' => strtolower(str_replace(' ', '-', $program->getCategory()->getName())),
            'hyphenizedProgram' => strtolower(str_replace(' ', '-', $program->getTitle()))
        ]);
    }
    ...
    {
        $category = $categoryRepository->findOneBy(['name' => ucwords(str_replace('-', ' ', $categoryName))]);
